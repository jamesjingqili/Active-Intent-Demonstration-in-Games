...
initial_state_truth = vcat(initial_state, SVector(θ, θ, 0.5))
initial_state_1 = vcat(initial_state, SVector(initial_belief, initial_belief, 0.5))
...
# ground_truth_costs_player_1 = sum([costs[1](g, x_true.x[t], x_true.u[t], t)[1] for t in 1:game_horizon])
# ground_truth_costs_player_2 = sum([costs[2](g, x_true.x[t], x_true.u[t], t)[1] for t in 1:game_horizon])
...
# savefig("ilqr_teaching_robot.png")
...
savefig("ilqr_teaching_robot_belief.png")
...
savefig("ilqr_teaching_robot_angle.png")
...
savefig("ilqr_teaching_robot_traj.png")
...
0, # x[2] is the y position of the robot
(u_list[Int(floor(t/ΔT))+1][2] - π1_P_list[Int(floor(t/ΔT))+1][2,[1,2,3,6]]'*[x-x_list[Int(floor(t/ΔT))+1]][1][[1,2,3,7]]-π1_α_list[Int(floor(t/ΔT))+1][2]-
(u_list[Int(floor(t/ΔT))+1][4] - π1_P_list[Int(floor(t/ΔT))+1][4,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π1_α_list[Int(floor(t/ΔT))+1][4]))*cos(x[3]) + 
(u_list[Int(floor(t/ΔT))+1][1] - π1_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,6]]'*[x-x_list[Int(floor(t/ΔT))+1]][1][[1,2,3,7]]-π1_α_list[Int(floor(t/ΔT))+1][1]-
(u_list[Int(floor(t/ΔT))+1][3] - π1_P_list[Int(floor(t/ΔT))+1][3,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π1_α_list[Int(floor(t/ΔT))+1][3]))*sin(x[3]), # x[3] is the heading angle of the robot
...
-1/ΔT*x[5] + 1/ΔT*x[2] + 1/ΔT * L*sin(x[3]) + (u_list[Int(floor(t/ΔT))+1][3] - π1_P_list[Int(floor(t/ΔT))+1][3,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π1_α_list[Int(floor(t/ΔT))+1][3]), # x[5] is the y position of human
0, # x[6] is the ground truth state 
0,  # mean 
0   # variance
) + SVector(  
...
costs3 = (
    FunctionPlayerCost((g, x, u, t) -> ( (x[1])^2 + (x[2])^2 +  10*(x[3] - x[7])^2 + (u[1])^2 + (u[2])^2 + σ1*(
        (u_list[Int(floor(t/ΔT))+1][1] - π1_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,6]]'*[x-x_list[Int(floor(t/ΔT))+1]][1][[1,2,3,7]]-π1_α_list[Int(floor(t/ΔT))+1][1])^2 + 
        (u_list[Int(floor(t/ΔT))+1][2] - π1_P_list[Int(floor(t/ΔT))+1][2,[1,2,3,6]]'*[x-x_list[Int(floor(t/ΔT))+1]][1][[1,2,3,7]]-π1_α_list[Int(floor(t/ΔT))+1][2])^2) )),  # robot cost
    FunctionPlayerCost((g, x, u, t) -> ( 0*(x[3] - x[6])^2 + σ2*((
        u[3]
        )^2 + (
        u[4]
        )^2) )) # human cost
...
push!(x3_list, x3.x[game_horizon]);
push!(u3_list, x3.u[game_horizon]);
...
x32_FB, y32_FB = [x3.x[i][4] for i in 1:game_horizon], [x3.x[i][5] for i in 1:game_horizon];
...
hline!([θ],label="target")
savefig("no_learning_robot_angle.png")




scatter(x31_FB, y31_FB,color=:black,label="robot")
scatter!(x32_FB, y32_FB,color=:red,label="human")
for t in 1:game_horizon
    if t == 1
        plot_label = "desk"
    else
    ...
    plot!([x31_FB[t], x32_FB[t]], [y31_FB[t], y32_FB[t]], color=:orange, label=plot_label)
end
savefig("no_learning_robot_traj.png")
