...
costs = (FunctionPlayerCost((g, x, u, t) -> (10*(x[1]-1)^2 + (x[4]-1)^2 + u[1]^2 + u[2]^2 - 0.2*((x[1]-x[5])^2 + (x[2]-x[6])^2))),
         FunctionPlayerCost((g, x, u, t) -> (   (x[5]-1)^2 + (x[8]-1)^2 + u[3]^2 + u[4]^2- 0.2*((x[1]-x[5])^2 + (x[2]-x[6])^2))))
...
solver1 = iLQSolver(g, max_scale_backtrack=10, max_elwise_diff_step=Inf,max_elwise_diff_converged=0.05, equilibrium_type="OLNE_costate")
x0 = SVector(0, 0, pi/2, 1.2,       1, 0, pi/2, 1)
converged1, trajectory1, strategies1 = solve(g, solver1, x0)


solver2 = iLQSolver(g,max_scale_backtrack=10, max_elwise_diff_step=Inf, max_elwise_diff_converged=0.05, equilibrium_type="FBNE_costate")
...
gif(anim1, "car_OLNE_costate_LineSearch.gif", fps = 10)
...
gif(anim2, "car_FBNE_costate_LineSearch.gif", fps = 10)
