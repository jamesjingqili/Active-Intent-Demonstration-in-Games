...
for index in 1:1 # three information patterns
    for jj in 1:num_noise_level
        mean_predictions1[index][jj] = mean(reduce(vcat,[optim_loss_list_list[ii][jj][index] for ii in 1:num_clean_traj]))
        variance_predictions1[index][jj] = var(reduce(vcat,[optim_loss_list_list[ii][jj][index] for ii in 1:num_clean_traj]))
    end
end
